...
#        return get_object_or_404(AnuncianteProfile, user=self.request.user)
    def get_object(self):
        if hasattr(self.request.user, 'alunoprofile'):
            raise Http404("Este usuário é aluno, não anunciante.")
        profile, _ = AnuncianteProfile.objects.get_or_create(user=self.request.user)
        return profile
...
class ConfigLoader:
    def __init__(self, config_path="/etc/config.json"):
